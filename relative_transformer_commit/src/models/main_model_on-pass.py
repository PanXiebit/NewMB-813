...
class MainModel(nn.Module):
    ...
    def forward(self, x, y):
        ...
        y_init = torch.zeros((bs, 1, h)).to(y.device)
        y_inp = torch.cat([y_init, y[:, :-1, :]], dim=1)
    ...
    def decoder_step_by_step(self, x):
        x = self.cnnbackbone(x)
        ...
        dec_inp = y_init = torch.zeros((bs, 1, h)).to(x.device)
        ...
        for i in range(24):
            ...
            # print(i, dec_out.shape, dec_inp.shape, dec_out[:, -1:, :].shape)
